...
public class RequestFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
		...
		logger.info("Authenticating jwtToken in request fiter.");
		try {
			...
			if (responseEntity.getStatusCode() == HttpStatus.OK) {
				logger.info("Authentication of jwtToken passed in request fiter.");
				filterChain.doFilter(request, response);
			} else {
				throw new Exception("Bad response: " + responseEntity.getStatusCode());
			}
		} catch (Exception e) {
			logger.warn("Authentication of jwtToken failed in request fiter.");
			e.printStackTrace();
